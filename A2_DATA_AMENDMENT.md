...
The earlier listings CSV must not be used as the A2 analysis dataset.
...
2. recompute the specification-required price and bathroom variables from that original data;
...
## Exact scope of the earlier-assignment references

The amendment prohibits the cleaned/modified **dataset**. It does not forbid
the transformations that the current specification explicitly requires.
...
controlled amenity indicators are implemented in the A2 workflow. No earlier
cleaned CSV is used as input.
